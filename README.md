...
```bash
LEPTOS_OUTPUT_NAME=pythonathon cargo lambda build --no-default-features --features=ssr --release
```

This should produce a binary at `target/lambda/pythonathon/bootstrap`.
